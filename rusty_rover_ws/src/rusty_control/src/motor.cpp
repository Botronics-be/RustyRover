...
bool Motor::setVelocity(double velocity) {
  ...
  if (std::fabs(velocity) < 0.001) {
    if (!i2c_->setDutyCycle(pos_pin, 0xFFFF)) {
      return false;
    }
    if (!i2c_->setDutyCycle(neg_pin, 0xFFFF)) {
      return false;
    }
    return true;
  }
  ...
  if (velocity > 0) {
    ...
    if (!i2c_->setDutyCycle(neg_pin, 0xFFFF - duty_cycle)) {
      return false;
    }
  } else {
    if (!i2c_->setDutyCycle(pos_pin, 0xFFFF - duty_cycle)) {
      return false;
    }
    if (!i2c_->setDutyCycle(neg_pin, 0xFFFF)) {
      return false;
